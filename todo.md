
# Todo List

- [x] Spec 1 : Rechercher la ou les salles attribuées à un cours
- [x] Spec 2 : Afficher la capacité maximale d'une salle
- [x] Spec 3 : Visualiser les créneaux où une salle est libre sur la semaine (ou plutôt où elle est utilisée ce qui revient au même)
- [x] Spec 4 : Rechercher les salles disponibles sur un horaire donné
- [x] Spec 5 : Générer un fichier ICS des cours suivis par un usager
- [x] Spec 6 : Détecter les chevauchements interdits dans les données
- [x] Spec 7 : Générer des statistiques de taux d'utilisation des salles
- [x] Spec 8 : Lister les salles selon leur capacité d'acceuil
- [x] Spec 9 : Identifier les salles surexploitées ou sous-utilisées

- [x] Spec NF 1 : Interface CLI claire, messages d'erreur explicites
- [x] Spec NF 2 : Réponse en =< 2 secondes pour toutes les requêtes
- [x] Spec NF 3 : ICS compatible Google Calendar / Outlook
- [ ] Spec NF 4 : Aucune duplication ni recouvrement de salle

- [x] Remplir le README