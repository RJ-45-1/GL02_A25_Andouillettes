...
cli
	...
	.argument('<startDate>', 'The start date of the semester in YYYY-MM-DD format')
	.argument('<endDate>', 'The end date of the semester in YYYY-MM-DD format')
	.action(({ args, options, logger }) => {

		const { path, output, startDate, endDate } = args;
				...
				fs.readFile(file, 'utf8', (err, data) => {
					...
					if (filesProcessed === files.length) {
						if (masterAnalyzer.parsedCourse.length > 0) {
							const icsData = masterAnalyzer.exportToICS(args.startDate, args.endDate);
